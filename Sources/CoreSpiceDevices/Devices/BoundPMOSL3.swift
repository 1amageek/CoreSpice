...
/// A PMOS Level 3 MOSFET bound to circuit nodes.
///
/// Adds mobility degradation (`theta`), DIBL (`eta`), and velocity
/// saturation (`kappa`, `vmax`) using source-referenced voltages.
public struct BoundPMOSL3: BoundDevice, VoltageLimitingDevice, TransientStateCommittingDevice, NoisyDevice, Sendable {
    ...
    private func operatingPoint(state: SolutionState) -> OperatingPointResult {
        ...
        let vdsatDenom = 1.0 + kappaEff * vsgOverdrive
        let vdsat = vsgOverdrive / vdsatDenom
        ...
        if vsd < vdsat {
            ...
            let clm = 1.0 + lambda * vsd
            let f = vsgOverdrive * vdsat - 0.5 * vdsat * vdsat
            let dF_dvgst = vdsat + (vsgOverdrive - vdsat) * dvdsat_dvgst
            isd = betaEff * f * clm
            gm = (dbeta_dvgst * dvsgOverdrive) * f * clm
                + betaEff * dF_dvgst * dvsgOverdrive * clm
            gds = (dbeta_dvgst * dvsgOverdrive_dVsd) * f * clm
                + betaEff * dF_dvgst * dvsgOverdrive_dVsd * clm
                + betaEff * f * lambda
        }
    ...
    private func meyerCapacitances(state: SolutionState) -> MeyerCapacitances {
        ...
        if vsgOverdrive <= 0 {
            ...
            let kappaEff = kappaBase + kappaVelocity
            let vdsat = vsgOverdrive / (1.0 + kappaEff * vsgOverdrive)
