...
/// Descriptor for an NMOS Level 3 MOSFET.
///
/// Adds mobility degradation (`theta`), DIBL (`eta`), and velocity
/// saturation (`kappa`, `vmax`).
public struct NMOSL3Descriptor: DeviceDescriptor, Sendable {

    public let typeName = "nmos_l3"
    ...
    public let parameterDescriptors = [
        ...
        ParameterDescriptor(name: "kappa", defaultValue: .real(0.0), description: "Velocity saturation coefficient (1/V)"),
        ParameterDescriptor(name: "vmax", defaultValue: .real(0.0), description: "Saturation velocity (m/s)"),
    ...
    private func extractModelParameters(from instance: Instance) throws -> MOSFETModelParameters {
        ...
        if let v = try extractReal("theta") { params.theta = v }
        if let v = try extractReal("eta") { params.eta = v }
        if let v = try extractReal("kappa") { params.kappa = v }
        if let v = try extractReal("vmax") { params.vmax = v }
