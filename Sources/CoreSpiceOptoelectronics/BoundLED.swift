...
/// A light-emitting diode (LED) bound to circuit nodes and an optical output node.
///
/// The electrical model is a standard PN junction (Shockley equation).
/// The optical output is proportional to forward current:
///   `P_out = eta_ext * I_fwd * h*nu/q`
///
/// Implements `OpticalEmitter` to provide local sensitivities `dP/dV`
/// for forward-mode sensitivity propagation through the optical DAG.
public struct BoundLED: OpticalEmitter, VoltageLimitingDevice, TransientStateCommittingDevice, Sendable {

    public let instance: Instance
    private let anode: Node
    private let cathode: Node
    private let parameters: LEDModelParameters

    private let anodeIdx: Int?
    private let cathodeIdx: Int?

    /// Optical output node where emitted light appears.
    ...
    public func computeOpticalOutputWithSensitivity(
        ...
        // Optical power: P = factor * max(I_fwd, 0)
        let factor = parameters.powerPerCurrent
        let iFwd = max(op.id, 0)
        let power = factor * iFwd
        let amplitude = power.squareRoot()

        let signal = OpticalSignal.fromPolar(
            amplitude: amplitude, phase: 0, wavelength: parameters.peakWavelength
        )
    ...
    // MARK: - OptoelectronicDevice Stamps
    ...
    public func stampDC(
        ...
        // Standard PN junction linearization (LED is primarily an electrical device)
